...
@section('content')
    <div class="max-w-xl mx-auto p-6">
        ...
        <form action="{{ route('pengunjung.store') }}" method="POST" class="bg-white p-6 rounded-xl shadow-lg space-y-4">
            @csrf

            <div>
                <label class="block mb-1 font-semibold text-gray-700">Tanggal</label>
                <input type="date" name="tanggal"
                    class="border rounded-lg p-2 w-full focus:ring-2 focus:ring-blue-400 focus:outline-none" required>
            </div>

            <div>
                <label class="block mb-1 font-semibold text-gray-700">Sumbangan Motor</label>
                <input type="number" id="motor" name="sumbangan_motor"
                    class="border rounded-lg p-2 w-full focus:ring-2 focus:ring-blue-400 focus:outline-none" value="0"
                    min="0">
            </div>
            ...
            <div>
                ...
                <input type="text" id="total_sumbangan"
                    class="border rounded-lg p-2 w-full bg-gray-100 font-semibold text-green-700" readonly>
            </div>

            <div>
                <label class="block mb-1 font-semibold text-gray-700">Total Pengunjung</label>
                <input type="text" id="total_pengunjung"
                    class="border rounded-lg p-2 w-full bg-gray-100 font-semibold text-blue-700" readonly>
            </div>
    ...
    <script>
        ...
        function updateTotal() {
            ...
            let totalSumbangan = (motor * 2000) + (mobil * 5000);
